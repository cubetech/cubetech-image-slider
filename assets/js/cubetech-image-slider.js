...
jQuery(document).ready(function() {
	...
	if (hover_pause == 1) {
		// remove the previously set setInterval function when mouse pointer is hovered over the cubetech-image-slider unordered list
		jQuery('.cubetech-image-slider-container').hover(function() {
			clearInterval(timer)
		}, function() {
...
function slider(where) {
	...
	var item_width = jQuery('.cubetech-image-slider li').outerWidth();
	...
	if (where == 'left') {
		...
		// in the css file global.css, relative position of the unordered list cubetech-image-slider is set to -200 to match the image widths (plus margins if necessary), here that css property is being added or subtracted with the image width depending on the direction of the carousel flow
		var left_indent = parseInt(jQuery('.cubetech-image-slider').css('left')) + item_width;
	} else {
		var left_indent = parseInt(jQuery('.cubetech-image-slider').css('left')) - item_width;
	}
	...
	jQuery('.cubetech-image-slider:not(:animated)').animate({'left': left_indent}, 500, function() {
		...
		if (where == 'left') {
			jQuery('.cubetech-image-slider li:first').before(jQuery('.cubetech-image-slider li:last'));
		} else {
			jQuery('.cubetech-image-slider li:last').after(jQuery('.cubetech-image-slider li:first'));
		}
		...
		if (matchMedia('(min-width: 321px) and (max-width: 767px)').matches){
			...
			jQuery('.cubetech-image-slider').css({'left' : '-750px'});
  		} 
